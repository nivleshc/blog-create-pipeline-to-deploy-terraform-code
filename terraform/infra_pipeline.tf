...
resource "aws_codebuild_project" "infra_plan_project" {
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "${var.project}_${var.env}_infra_codebuildloggroup"
      stream_name = "${var.project}_${var.env}_infra_codebuildlogstream"
    }
  }
...
resource "aws_sns_topic_subscription" "infra_owner_subscription" {
